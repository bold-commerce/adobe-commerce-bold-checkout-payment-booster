...
class DeactivateQuotes extends Value
{
    ...
    public const CRON_MODEL_PATH =
        'crontab/default/jobs/bold_booster_deactivate_digital_wallets_quotes/run/model';
    ...
    public function beforeSave(): self
    {
        ...
        if ($frequency === 'hourly' && ($occurrence < 1 || $occurrence > 23)) {
            throw new LocalizedException(__('%d is not a valid hourly occurrence', $occurrence));
        }

        if ($frequency === 'daily' && ($occurrence < 1 || $occurrence > 31)) {
            throw new LocalizedException(__('%d is not a valid daily occurrence', $occurrence));
        }
    ...
    public function afterSave(): self
    {
        ...
        /** @var string $sequence */
        $sequence = $this ->_config->getValue(
